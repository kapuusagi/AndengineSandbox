...
public final class AndEngine {
	...
	/**
	 * デバイスがサポートされているかどうかを判定する。
	 * @param pContext コンテキスト
	 * @return デバイスがサポートされている場合にはtrue
	 */
	...
	/**
	 * デバイスがサポートされているかどうかをチェックする。
	 * デバイスがサポートされていない場合にはDeviceNotSupportedExceptionが投げられる。
	 * @param pContext コンテキスト
	 * @throws DeviceNotSupportedException デバイスがサポートされていない場合。
	 */
	...
	private static void checkCodePathSupport() throws DeviceNotSupportedException {
		if (SystemUtils.isAndroidVersionOrLower(Build.VERSION_CODES.FROYO)) {
			// Android Version.2.2(FROYO) 以下の場合
			try {
